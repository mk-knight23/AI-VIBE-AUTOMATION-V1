...
export const executeWorkflow = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Security: For non-webhook triggers, verify user owns the workflow
    if (trigger !== "webhook" && userId) {
      await step.run("verify-ownership", async () => {
        ...
        if (workflow.userId !== userId) {
          throw new Error("Forbidden: User does not own this workflow");
        }
      });
    }
    ...
    const workflow = await step.run("fetch-workflow", async () => {
      ...
      // Additional security check for webhook triggers
      if (trigger === "webhook" && !wf.webhookEnabled) {
        throw new Error("Webhook is not enabled for this workflow");
      }
    ...
    try {
      ...
      while (currentNode && iterations < MAX_ITERATIONS) {
        ...
        await step.run(
          `log-node-success-${node.id}-${iterations}`,
          async () => {
            ...
            await prisma.executionLog.create({
              data: {
                ...
                level: "INFO",
                data: nodeOutput
                  ? JSON.parse(JSON.stringify(nodeOutput))
                  : undefined,
              },
            });
          },
        ...
        if (node.type === "ifelse") {
          ...
          const edge = nodeEdges.find(
            (e) => e.sourceHandle === (isFinished ? "done" : "loop"),
          );
          nextNodeId = edge?.target || null;
        } else {
          // Linear flow
      ...
      if (iterations >= MAX_ITERATIONS) {
        throw new Error(
          "Max iterations reached. Possible infinite loop detected.",
        );
      }
